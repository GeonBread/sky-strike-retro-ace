...
export function GameOverPanel({ onShare, onLeaderboard }: GameOverPanelProps) {
  ...
  const submitAllScores = async () => {
    ...
    let onlineStatus: ChannelStatus = onlineReady ? "idle" : "skipped";
    let localRank: number | null = null;
    let onlineRank: number | null = null;
    ...
    try {
      ...
      localRank = await localLeaderboard.getRank(localEntry.score);
      localStatus = "done";
      messages.push("로컬 등록 완료");
    } catch (error) {
      localStatus = "error";
      messages.push(error instanceof Error ? `로컬 실패: ${error.message}` : "로컬 등록 실패");
    }
    ...
    if (onlineReady) {
      try {
        ...
        onlineRank = await onlineLeaderboard.getRank(onlineEntry.score);
        onlineStatus = "done";
        messages.push("온라인 등록 완료");
      } catch (error) {
        onlineStatus = "error";
        messages.push(error instanceof Error ? `온라인 실패: ${error.message}` : "온라인 등록 실패");
      }
    } else {
      messages.push("온라인 설정 없음");
    }

    const hasAnySuccess = localStatus === "done" || onlineStatus === "done";
    setStatus(hasAnySuccess ? "done" : "error");
    setResult({
      local: localStatus,
      online: onlineStatus,
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="text-sm font-semibold text-slate-400 mb-6">최종 점수</div>
      <div className="text-6xl font-mono font-black text-white mb-6 tracking-wider">{score.toString().padStart(6, "0")}</div>
      ...
      <div className="w-full mb-4">
        ...
        <input
          ...
          className="w-full h-12 bg-slate-950 border border-slate-800 focus:border-cyan-400 disabled:opacity-60 outline-none rounded-lg px-4 font-mono text-slate-100 text-center uppercase tracking-wider"
        />
      </div>
      ...
      <div className="min-h-20 w-full mb-6 space-y-2">
        {result.message && (
          <div className={`text-xs font-semibold text-center flex items-center justify-center gap-2 ${status === "error" ? "text-rose-400" : "text-emerald-300"}`}>
